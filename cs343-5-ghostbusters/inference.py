...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            allPossible[self.getJailPosition()] = 0.0
        for legalPos in self.legalPositions:
            trueDistance = util.manhattanDistance(legalPos, pacmanPosition)
            allPossible[legalPos] = emissionModel[trueDistance] * self.beliefs[legalPos]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for legalPos in self.legalPositions:
            if self.setGhostPosition(gameState, legalPos) is not None:
                posDist = self.getPositionDistribution(self.setGhostPosition(gameState, legalPos))
                for pos in posDist:
                    if pos is not None:
                        allPossible[pos] += posDist[pos] * self.beliefs[legalPos]
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        for particle in set(self.particles):
            beliefs[particle] = self.particles.count(particle) + scale
        beliefs.normalize()
        return beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        ghosts = self.numGhosts
        legalPositions = self.legalPositions
        possibleComb = list(itertools.product(legalPositions, repeat=ghosts))
        random.shuffle(possibleComb)
        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(possibleComb[i % len(possibleComb)])
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            for j in range(self.numGhosts):
                if noisyDistances[j] != None:
                    weight *= emissionModels[j][util.manhattanDistance(self.particles[i][j], pacmanPosition)]
        ...
        for i in range(self.numParticles):
            if weights[i] != 0:
                weightChanged = True
                distribution[self.particles[i]] += weights[i]

        if not weightChanged:
            self.initializeParticles()
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i, oldPos in enumerate(newParticle):
