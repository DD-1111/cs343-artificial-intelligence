...
def enhancedFeatureExtractorDigit(datum):
    ...
    Count for different direction and mark the direction with most count.
    Count for the clusters and mark the corresponding pixel
    ##
    """
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            ...
            if datum.getPixel(x, y) > 0:
                ...
                if search(x,y):
                    #print ("wdwdwd-wd--")
                    features [str(x) + str(y)] = 1
                    count += 1
                else:
                    #print("--------")
                    features[str(x) + str(y)] = 0


    mostDirected = directCounter.index(max(directCounter))
   # print ('most = '+str(mostDirected))
    for i in range(4):
        #print(i)
        if (mostDirected) != i:
            features[str(i)] = 0
...
def enhancedPacmanFeatures(state, action):
    ...
    ghostPos = successor.getGhostPositions()
    nearestGhostDis = min(util.manhattanDistance(ghost, pacmanPos) for ghost in ghostPos)
    if nearestGhostDis > 0:
        features['nearestghostdistance'] = 1.0/nearestGhostDis
    else:
        features['nearestghostdistance'] = 0
    currentFood = successor.getFood()
    foodPos = currentFood.asList()
    if currentFood.count() > 0:
        nearestFoodDis = min(util.manhattanDistance(food, pacmanPos) for food in foodPos)
        features["nearestfooddistance"] = 1.0/ nearestFoodDis
        features['foodcount'] = 1.0/currentFood.count()
    else:
